...
class AlgoholismFilterSuite extends ScalatraFunSuite with ShouldMatchers {
  ...
  test("POST / returns knapsack with item that fits") {
    post("/", """{ "name" : "Large contents", "timeout" : 15000, "contents":[{"id":"1","weight":[8180,9604],"value":441},{"id":"2","weight":[349,463],"value":400},{"id":"3","weight":[9534,9462],"value":440}],"capacity":[2000,2000] }""", Map[String,String]()) {
      status should equal (200)
      body should equal("""["2"]""" + "\n")
    }
  }
  ...
  test("POST / returns knapsack with several items that fit together") {
    post("/", """{ "name" : "Large contents", "timeout" : 15000, "contents":[{"id":"1","weight":[8180,9604],"value":441},{"id":"2","weight":[349,463],"value":400},{"id":"3","weight":[123,98],"value":440}],"capacity":[2000,2000] }""", Map[String,String]()) {
      status should equal (200)
